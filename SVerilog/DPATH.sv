...
module datapath (
    ...
    input  logic         flags_reset,    // Synchronous active-high reset for status flags
    input  logic [3:0]   alu_op,         // ALU operation select
    input  logic         alu_cin,        // Carry-in for ALU

    // --- Memory-Mapped I/O Control (Unified with Memory Access) ---
    input  logic         mem_write_en,   // Unified memory/IO write enable
    input  logic         mem_read_en,    // Unified memory/IO read enable
    input  logic [7:0]   mem_addr_bus,   // Unified address bus for Memory and I/O

    // --- Data Busses & External I/O ---
 input  logic [7:0]  data_bus_in,      // da UC
    input  logic [7:0]  mem_data_in,      // dado lido da RAM
    input  logic        sel_mem_data,
    ...
    output logic [7:0]   mem_read_data,  // Data read from Memory or I/O
    ...
    //MUX
logic [7:0] ac_data_in;
assign ac_data_in = (sel_mem_data) ? mem_data_in : data_bus_in;
    // =================================================================
    // Sequential Logic
    // =================================================================
always @(posedge clk) begin
        if (reset) begin
            ...
            // --- I/O Write (LEDs) ---
               if (mem_write_en && mem_addr_bus == 8'h00) begin // Changed io_write_en to mem_write_en and addr_bus_in to mem_addr_bus
                led_reg <= ac_reg[3:0];
            end
    ...
    // =================================================================
    // Memory-Mapped I/O Read Logic
    // =================================================================
    always @(*) begin
        mem_read_data = 8'h00; 
        if (mem_read_en) begin
            case (mem_addr_bus)
                // Define an address for the Switches, e.g., 8'hF4
                8'hF4: mem_read_data = {4'b0000, in_switches_sync};
                // Add other memory-mapped devices here
                // For now, if not an I/O address, it would be a memory read.
                // A full system would integrate an actual memory module here.
                default: mem_read_data = 8'h00; // Or from a memory array
            endcase
        end
    end
